...
(define (tokeniser str)
  ...
  (do-until (= i (length input)) ; Loop through the chars until the end of the input list
    (let ((char (list-ref input i)))
      (cond ; Match any
        ((char=? #\newline char) (set! toks (add-token toks "NEWLINE" "\n")))
        ((char=? char #\tab) (set! toks (add-token toks "TAB" "\t")))
        ((char=? char #\-) (if (char=? (list-ref input (+ i 1)) #\>)
...
(define (check-number input lst)
 ...
 (let ((num (string->number (list->string char-list))))
  (append lst `(,(make-token type: "NUMBERLITERAL" value: num)))))

(define (check-string input lst)
 (define char-list '())
 (define delim (list-ref input i))
 (set! i (+ i 1))
 (define char (list-ref input i))
 (do-until (char=? char delim)
  ...
  (set! i (+ i 1))
  (set! char (list-ref input i)))
 (append lst `(,(make-token type: "STRINGLITERAL" value: (list->string char-list)))))
